...
#define SAMPLE_DEVICE_NAME          "GROUP_6"
...
#define I2C_SLAVE_SDA_IO 11              /*!< gpio number for i2c slave data */
...
#define I2C_SLAVE_TASK_PRIORITY     10                         /*!< I2C master doesn't need buffer */
...
void task_ssd1306_display_text(char txt[100]) {
	char *text = txt;
...
void task_ssd1306_display_clear(void *ignore) {
    ...
    // Delete the task since we only want it to run once
    vTaskDelete(NULL);
}


void task_ssd1306_display_bitmap(const uint8_t *const_bitmap)
{
    uint8_t *bitmap = (uint8_t *)const_bitmap;
    const uint8_t cmd_mode[] = {0x20, 0x21 };
    const uint8_t cmd_startline[] = {0xA1};
    const uint8_t cmd_direc[] = {0xC8 };
    const uint8_t cmd_column_range[] = {0x21, 0x00, 0x7F};
    const uint8_t cmd_page_range[] = {0x22, 0x00, 0x07};
    ...
    i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_CMD_STREAM, true);
    i2c_master_write(cmd, cmd_mode, sizeof(cmd_mode), true);
    i2c_master_write(cmd, cmd_page_range, sizeof(cmd_page_range), true);
    i2c_master_write(cmd, cmd_column_range, sizeof(cmd_column_range), true);
    i2c_master_write(cmd, cmd_startline, sizeof(cmd_startline), true);
    i2c_master_write(cmd, cmd_direc, sizeof(cmd_direc), true);
    i2c_master_stop(cmd);
	i2c_master_cmd_begin(I2C_NUM_1, cmd, 1000/portTICK_PERIOD_MS);
	i2c_cmd_link_delete(cmd);
    ...
    i2c_master_write_byte(cmd, (OLED_I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, OLED_CONTROL_BYTE_DATA_STREAM, true);
    i2c_master_write(cmd, bitmap, 1024, true);
    i2c_master_stop(cmd);
	i2c_master_cmd_begin(I2C_NUM_1, cmd, 1000/portTICK_PERIOD_MS);
	i2c_cmd_link_delete(cmd);
    ...
    switch (event) {
        ...
        case ESP_GAP_BLE_AUTH_CMPL_EVT: {
            ...
            if (param->ble_security.auth_cmpl.success){
                ...
                ESP_LOGI(EXAMPLE_TAG, " Please input ID of each team member:\n");
            }
            else {
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if(gatt_db_handle_table[IDX_CHAR_VAL_A] == param->write.handle && param->write.len == 4) {
                    ...
                    xTaskCreate(&task_ssd1306_display_clear, "ssd1306_clear", 2048, NULL, 5, NULL);
                    vTaskDelay(2000 / portTICK_PERIOD_MS);
                    uint8_t *value = param->write.value;
                    size_t num_values = 4;
                    char hex_string[num_values * 2 + 1]; // +1 for null-terminator

                    convert_to_hex_str(hex_string, value, num_values);
                    hex_string[num_values * 2] = '\0'; // Null-terminate the string

                    xTaskCreate(task_ssd1306_display_text, "ssd1306_display_text", 2048, hex_string, 10, NULL);
                    printf("Gia tri da duoc ghi la: %02x %02x %02x %02x\n", value[0], value[1], value[2], value[3]);
